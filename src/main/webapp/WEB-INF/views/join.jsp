		...
		<form class="form" name="frmJoin" id="frmJoin" method="POST" />
			...
			<ul>
				...
				<li class="addrFinder">
					...
					<input type="text" id="ma_addr2" name="ma_addr2" placeholder="상세주소" maxlength="30" style="width:300px;" readonly />
...
<% } %>
				...
				<li>
					<p>이용약관</p>
					<p id="agreement" style="width:800px; height:200px; overflow:scroll; overflow-x: hidden; text-align:start;">
					...
					<p><input type="hidden"/></p><p><input type="hidden"/></p><p><input type="hidden"/></p>
					...
					<p id="agreement" style="width:800px; height:200px; overflow:scroll; overflow-x: hidden; text-align:start; margin-top:50px;">
					...
					<input type="checkbox" name="agree" value="y" id="agreeY" class="privacy">
						...
						<p><input type="hidden"/></p>
					<p>
					<a><input name="agreeAll" type="checkbox" >전체동의</a>
					</p>
				</li>
			</ul>
...
<script>
    ...
    //2개 체크중 하나라도 체크해제하면 전체체크도 같이 해제
    $('input[class=useAgree]').on('click',function(){
        if(!$('input[class=useAgree]').prop('checked')){
